...
<div class="wrap cqa-wrap">
    ...
    <form method="post" action="" class="cqa-school-form">
            ...
            <div class="cqa-card">
                ...
                <div class="cqa-card-body">
                    <table class="form-table">
                        <tr>
                            <th><label for="name"><?php esc_html_e('School Name', 'chroma-qa-reports'); ?> <span
                                        class="required">*</span></label></th>
                            <td>
                                <input type="text" id="name" name="name"
                                    value="<?php echo esc_attr($school->name ?? ''); ?>" class="regular-text"
                                    required>
                            </td>
                        </tr>
                        <tr>
                            <th><label
                                    for="location"><?php esc_html_e('Location/Address', 'chroma-qa-reports'); ?></label>
                            </th>
                            <td>
                                <input type="text" id="location" name="location"
                                    value="<?php echo esc_attr($school->location ?? ''); ?>" class="regular-text"
                                    list="locations-list">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" id="region" name="region"
                                    value="<?php echo esc_attr($school->region ?? ''); ?>" class="regular-text"
                                    list="regions-list">
                                ...
                                <p class="description">
                                    <?php esc_html_e('Enter a region name or select from existing', 'chroma-qa-reports'); ?>
                                </p>
                            </td>
                        </tr>
                        <tr>
                            <th><label
                                    for="acquired_date"><?php esc_html_e('Acquired Date', 'chroma-qa-reports'); ?></label>
                            </th>
                            <td>
                                <input type="date" id="acquired_date" name="acquired_date"
                                    value="<?php echo esc_attr($school->acquired_date ?? ''); ?>">
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <select id="status" name="status">
                                    <option value="active" <?php selected($school->status ?? 'active', 'active'); ?>>
                                        <?php esc_html_e('Active', 'chroma-qa-reports'); ?></option>
                                    <option value="inactive" <?php selected($school->status ?? '', 'inactive'); ?>>
                                        <?php esc_html_e('Inactive', 'chroma-qa-reports'); ?></option>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            <th><label for="tier"><?php esc_html_e('Service Tier', 'chroma-qa-reports'); ?></label>
                            </th>
                            <td>
                                <select id="tier" name="tier">
                                    <option value="1" <?php selected($school->tier ?? 1, 1); ?>>
                                        <?php esc_html_e('Tier 1 (High Support)', 'chroma-qa-reports'); ?></option>
                                    <option value="2" <?php selected($school->tier ?? 1, 2); ?>>
                                        <?php esc_html_e('Tier 2 (Standard)', 'chroma-qa-reports'); ?></option>
                                    <option value="3" <?php selected($school->tier ?? 1, 3); ?>>
                                        <?php esc_html_e('Tier 3 (Maintenance)', 'chroma-qa-reports'); ?></option>
                                    <option value="4" <?php selected($school->tier ?? 1, 4); ?>>
                                        <?php esc_html_e('Tier 4 (Low Touch)', 'chroma-qa-reports'); ?></option>
                                </select>
                                <p class="description">
                                    <?php esc_html_e('Determines the frequency and depth of QA inspections.', 'chroma-qa-reports'); ?>
                                </p>
                            </td>
                        </tr>
                    </table>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" name="classroom[<?php echo esc_attr($key); ?>]"
                                            value="<?php echo esc_attr($current_value); ?>" min="0" max="20"
                                            style="width: 80px;">
                                    </td>
                                </tr>
                            <?php endforeach; ?>
            ...
            <div class="cqa-card">
                ...
                <div class="cqa-card-body">
                    <table class="form-table">
                        <tr>
                            <th><label
                                    for="drive_folder_id"><?php esc_html_e('Folder ID', 'chroma-qa-reports'); ?></label>
                            </th>
                            <td>
                                <input type="text" id="drive_folder_id" name="drive_folder_id"
                                    value="<?php echo esc_attr($school->drive_folder_id ?? ''); ?>"
                                    class="regular-text">
                                <p class="description">
                                    <?php esc_html_e('The Google Drive folder ID for this school\'s photos and documents.', 'chroma-qa-reports'); ?>
                                </p>
                            </td>
                        </tr>
                    </table>
        ...
        <p class="submit">
            ...
            <?php if (!$is_new): ?>
                <a href="<?php echo esc_url(admin_url('admin.php?page=chroma-qa-reports-create&school_id=' . $school->id)); ?>"
