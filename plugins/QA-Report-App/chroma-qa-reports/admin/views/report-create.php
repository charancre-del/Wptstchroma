...
// Get all active schools (unlimited for scalability buffer - QAR-048)
$schools = School::all(['status' => 'active', 'limit' => -1]);
?>
...
<div class="wrap cqa-wrap">
    <div class="cqa-header">
        ...
        <div class="cqa-header-actions">
            <a href="<?php echo esc_url(admin_url('admin.php?page=chroma-qa-reports-import')); ?>"
            ...
            <div class="cqa-card">
                ...
                <div class="cqa-card-body">
                    <table class="form-table">
                        <tr>
                            <th><label for="school_id"><?php esc_html_e('School', 'chroma-qa-reports'); ?> <span
                                        class="required">*</span></label></th>
                            <td>
                                <!-- Type-ahead Search (QAR-048) -->
                                <input type="text" id="school-search-filter" class="regular-text"
                                    placeholder="🔍 Type to search..."
                                    style="margin-bottom: 5px; width: 100%; max-width: 400px; display: block;"
                                    autocomplete="off">

                                <select id="school_id" name="school_id" class="regular-text" required size="5"
                                    style="height: auto; max-height: 200px;">
                                    <option value=""><?php esc_html_e('Select a school...', 'chroma-qa-reports'); ?>
                                    </option>
                        ...
                        <tr>
                            <th><label for="report_type"><?php esc_html_e('Report Type', 'chroma-qa-reports'); ?>
                                ...
                                <div class="cqa-report-types">
                                    <label class="cqa-type-option">
                                        ...
                                        <div class="cqa-type-card">
                                            ...
                                            <p><?php esc_html_e('Initial QA for newly acquired schools', 'chroma-qa-reports'); ?>
                                            </p>
                                        </div>
                                    </label>
                                    <label class="cqa-type-option">
                                        ...
                                        <div class="cqa-type-card">
                                            ...
                                            <p><?php esc_html_e('Standard QA inspection checklist', 'chroma-qa-reports'); ?>
                                            </p>
                                        </div>
                                    </label>
                                    <label class="cqa-type-option">
                                        ...
                                        <div class="cqa-type-card">
                                            ...
                                            <p><?php esc_html_e('Full inspection with CQI add-on', 'chroma-qa-reports'); ?>
                                            </p>
                                        </div>
                                    </label>
                                </div>
                        ...
                        <tr>
                            <th><label
                                    for="inspection_date"><?php esc_html_e('Inspection Date', 'chroma-qa-reports'); ?>
                                    <span class="required">*</span></label></th>
                            <td>
                                <input type="date" id="inspection_date" name="inspection_date"
                                    value="<?php echo esc_attr($report->inspection_date ?? date('Y-m-d')); ?>"
                                    required>
                            </td>
                        </tr>
                        <tr>
                            <th><label
                                    for="previous_report_id"><?php esc_html_e('Compare With', 'chroma-qa-reports'); ?></label>
                            </th>
                            <td>
                                <select id="previous_report_id" name="previous_report_id" class="regular-text">
                                    <option value="">
                                        <?php esc_html_e('No comparison (first report)', 'chroma-qa-reports'); ?>
                                    </option>
                                    <?php
                                    // QAR-010: PHP Fallback for Previous Reports
                                    if ($selected_school_id) {
                                        $school_obj = School::find($selected_school_id);
                                        if ($school_obj) {
                                            $prev_reports = $school_obj->get_recent_reports(20);
                                            foreach ($prev_reports as $pr) {
                                                // Skip current report if editing
                                                if ($is_edit && $pr->id == $report_id)
                                                    continue;

                                                $pr_date = date_i18n(get_option('date_format'), strtotime($pr->inspection_date));
                                                $label = sprintf('%s - %s (%s)', $pr->get_type_label(), $pr_date, $pr->get_rating_label());
                                                ?>
                                                <option value="<?php echo esc_attr($pr->id); ?>" <?php selected($report->previous_report_id ?? 0, $pr->id); ?>>
                                ...
                                <p class="description">
                                    <?php esc_html_e('Select a previous report to enable comparison features.', 'chroma-qa-reports'); ?>
                                </p>
                            </td>
                        </tr>
                    </table>
            ...
            <div class="cqa-card">
                ...
                <div class="cqa-card-body">
                    ...
                    <div class="cqa-form-row">
                        <label
                            for="overall_rating"><?php esc_html_e('Overall Rating', 'chroma-qa-reports'); ?></label>
                        <div class="cqa-rating-options">
                            <label class="cqa-rating-option">
                                ...
                                <span class="cqa-rating-badge exceeds">✅
                                    <?php esc_html_e('Exceeds', 'chroma-qa-reports'); ?></span>
                            </label>
                            <label class="cqa-rating-option">
                                <input type="radio" name="overall_rating" value="meets" <?php checked($report->overall_rating ?? '', 'meets'); ?>>
                                <span class="cqa-rating-badge meets">☑️
                                    <?php esc_html_e('Meets', 'chroma-qa-reports'); ?></span>
                            </label>
                            <label class="cqa-rating-option">
                                <input type="radio" name="overall_rating" value="needs_improvement" <?php checked($report->overall_rating ?? '', 'needs_improvement'); ?>>
                                <span class="cqa-rating-badge needs">⚠️
                                    <?php esc_html_e('Needs Improvement', 'chroma-qa-reports'); ?></span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="cqa-form-row">
                        ...
                        <textarea id="closing_notes" name="closing_notes" rows="5"
                            class="large-text"><?php echo esc_textarea($report->closing_notes ?? ''); ?></textarea>
                    </div>
                </div>
            </div>
        </div>
...
<style>
    ...
    .cqa-photo-thumb select:focus,
    .cqa-photo-thumb input:focus {
