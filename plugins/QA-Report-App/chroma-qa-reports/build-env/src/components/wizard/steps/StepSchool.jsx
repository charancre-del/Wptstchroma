...
const StepSchool = ( { draft, updateDraft, nextStep, readOnly = false } ) => {
    ...
    const currentReportId = draft.id;
    ...
    useEffect( () => {
        const fetchReportsForSchool = async () => {
            ...
            try {
                ...
                const reports = rawReports.filter( ( r ) => parseInt( r.id ) !== parseInt( currentReportId ) );
    ...
    }, [ selectedSchool, selectedSchoolId, currentReportId, updateDraft, draft.previous_report_id ] );
                                ...
                                <div className="p-3 bg-indigo-50 border-l-4 border-cqa-primary flex justify-between items-center">
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-500">
                                            { selectedSchool.region } • Tier{ ' ' }
                                            { selectedSchool.tier !== undefined ? selectedSchool.tier : 1 }
                                        </div>
                                    </div>
                                    ...
                                    className={ `
                                        ...
                                        ${
                                            draft.school_id === school.id
                                                ? 'bg-indigo-50 border-l-4 border-cqa-primary'
                                                : ''
                                        }
                                    ` }
                                >
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-500">
                                            { school.region } • Tier { school.tier !== undefined ? school.tier : 1 }
                                        </div>
                                    </div>
                                    { draft.school_id === school.id && (
                                        <span className="text-cqa-primary font-bold text-sm">Selected</span>
                    ...
                    { readOnly ? (
                        <div className="bg-white p-3 rounded-md border border-blue-100 flex items-center gap-2">
                            ...
                            <span className="text-sm text-blue-800">
                                Linked to report from { draft.previous_report_date || 'N/A' }
                            </span>
                        </div>
                    ) : isFetchingReports ? (
                        ...
                        <div className="space-y-3">
                            <p className="text-sm text-blue-800">
                                Found <strong>{ allReports.length }</strong> previous report(s) for{ ' ' }
                                <strong>{ selectedSchool.name }</strong>. Select one to compare:
                            </p>

                            <div className="flex flex-col gap-2 max-h-48 overflow-y-auto">
                                { allReports.map( ( report ) => (
                                    <label
                                        htmlFor={ `report-link-${ report.id }` }
                                        key={ report.id }
                                        className={ `
                                    ...
                                    >
                                        ...
                                        <span
                                            className={ `px-2 py-0.5 rounded text-xs font-medium capitalize ${ getStatusBadge(
                                                report.status
                                            ) }` }
                                        >
                                            { report.status }
                                ...
                                { /* No Link Option */ }
                                <label
                                    htmlFor="report-link-none"
                                    aria-label="Do not link this report to a previous report"
                                    className={ `
                                    flex items-center gap-3 p-3 bg-white rounded-md border cursor-pointer transition-colors
                                    ${
