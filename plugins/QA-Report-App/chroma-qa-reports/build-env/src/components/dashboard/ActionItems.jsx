    ...
    return (
        <div className="bg-white rounded-3xl p-8 border border-brand-ink/5 shadow-sm h-full flex flex-col">
            ...
            <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                { items.length === 0 ? (
                    <div className="text-center py-8 text-brand-ink/40 text-sm font-bold">No pending actions.</div>
                ) : (
                    items.map( ( item ) => (
            ...
            <Link
                ...
                className="block text-center w-full mt-6 py-3 rounded-xl border border-brand-ink/10 text-brand-ink/60 font-bold text-xs uppercase tracking-wider hover:bg-brand-ink hover:text-white transition-colors"
            >
                View All Actions
            </Link>
        </div>
