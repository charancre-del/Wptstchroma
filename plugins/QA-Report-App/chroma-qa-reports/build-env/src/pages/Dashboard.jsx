...
import { FileText, School, CheckCircle, AlertTriangle, BarChart3, ArrowRight, Printer } from 'lucide-react';
        ...
        <div className="p-6 w-full">
            ...
            <section className="bg-white rounded-3xl border border-brand-ink/5 shadow-sm p-8">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <Link
                        to="/reports"
                        className="flex items-center gap-2 text-sm font-bold text-chroma-blue hover:text-chroma-blue/80 transition-colors"
                    >
                    ...
                    { reportsLoading ? (
                        <div className="space-y-4">
                            { [ 1, 2, 3 ].map( ( i ) => (
                                <div
                                    key={ i }
                                    className="p-4 rounded-xl border border-brand-ink/5 flex items-center justify-between"
                                >
                        ...
                        <div className="grid gap-4">
                            { recentReports.map( ( report ) => (
                                <div
                                    key={ report.id }
                                    className="p-4 rounded-xl bg-brand-cream/50 border border-brand-ink/5 hover:border-chroma-blue/20 hover:shadow-md transition-all flex items-center justify-between group"
                                >
                                    <div className="flex items-center gap-4">
                                        <div
                                    ...
                                    <div className="flex items-center gap-4">
                                        { report.rating && (
                                            <span
                                                ...
                                                ${
                                                    report.rating.includes( 'Exceeds' )
                                                        ? 'bg-chroma-green/10 text-chroma-green'
                                                        : report.rating.includes( 'Meets' )
                                                        ? 'bg-chroma-blue/10 text-chroma-blue'
                                                        : 'bg-chroma-red/10 text-chroma-red'
                                                }` }
                                            >
