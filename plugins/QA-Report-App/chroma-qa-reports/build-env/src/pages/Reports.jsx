...
export function ReportsPage() {
    ...
    const [ authorFilter ] = useState( initialAuthor );
    ...
    const canDeleteReport = useCallback(
        ( report ) => {
            ...
            return Boolean(
                capabilities?.cqa_delete_reports || ( capabilities?.cqa_delete_own_reports && report.is_mine )
            );
        },
        [ capabilities ]
    );
    ...
    const handleDeleteDraft = useCallback(
        async ( report ) => {
            ...
            try {
                ...
                addToast( { type: 'error', message: deleteError.message || 'Failed to delete draft' } );
            }
        },
    ...
    const filteredReports = useMemo( () => {
        return reports.filter( ( report ) => {
            ...
            if ( authorFilter === 'me' && report.is_mine !== true ) {
                return false;
            } // Assuming 'is_mine' or similar prop exists from API
            if ( schoolFilter && String( report.school_id ) !== String( schoolFilter ) ) {
                return false;
            }
            return true;
        } );
    ...
    const columns = useMemo(
        () => [
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-brand-ink/60 font-medium">
                                { row.original.report_type_label ||
                                    REPORT_TYPES[ row.original.report_type ] ||
            ...
            },
            {
                ...
                cell: ( { getValue } ) => {
                    ...
                    return (
                        <span
                            className={ cn(
                                'px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 w-fit uppercase tracking-wider',
                                config.color
                            ) }
                        >
            ...
            },
            {
                ...
                header: 'Author',
                cell: ( { getValue } ) => <span className="text-brand-ink/60 font-medium">{ getValue() || '—' }</span>,
            },
            {
                accessorKey: 'created_at',
                header: 'Created',
                cell: ( { getValue } ) => (
                    <span className="text-sm text-brand-ink/40 font-medium">{ formatDate( getValue() ) || '—' }</span>
                ),
            },
            {
                id: 'actions',
                                            ...
                                            <button
                                                ...
                                                className={ cn(
                                                    ...
                                                    header.column.getCanSort() &&
                                                        'cursor-pointer select-none hover:text-brand-ink transition-colors'
                                                ) }
