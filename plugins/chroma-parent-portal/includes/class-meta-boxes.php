...
class Chroma_Portal_Meta_Boxes
{
    ...
    {
        $screens = ['cp_lesson_plan', 'cp_meal_plan', 'cp_resource', 'cp_form', 'cp_announcement', 'cp_home_activity'];
    ...
    public function render_pdf_box($post)
    {
        ...
        <div style="padding: 10px;">
            <p>
                <input type="hidden" id="chroma_portal_pdf_id" name="chroma_portal_pdf_id"
                ...
                <button type="button" class="button button-link-delete" id="chroma_portal_remove_pdf_btn"
                    style="<?php echo $file_id ? '' : 'display:none;'; ?>">Remove</button>
            </p>
            <p id="chroma_portal_pdf_preview">
                <?php if ($file_url): ?>
                    <strong>Selected:</strong> <a href="<?php echo esc_url($file_url); ?>"
                        target="_blank"><?php echo basename($file_url); ?></a>
                <?php else: ?>
    ...
    public function render_pin_box($post)
    {
        ...
        <p>
            ...
            <input type="text" id="chroma_portal_new_pin" name="chroma_portal_new_pin" value=""
                placeholder="Leave empty to keep current" maxlength="6" pattern="\d{4,6}" />
        </p>
