...
(function () {
  ...
  const initLocationExplorer = (explorer) => {
    ...
    const applyFilter = (filter, labelOverride) => {
      ...
      if (filter === 'closest' && userCoords) {
        ...
        if (status) {
          status.textContent = 'Closest campuses sorted by distance.';
        }
      } else {
        cards.forEach((card) => {
          const distanceEl = card.querySelector('[data-location-distance]');
      ...
      const readableFilter = filter === 'closest'
        ? (userCoords ? 'Closest to your browser location' : '')
        : (labelOverride || 'Filtered by live campus region');
    ...
    const requestClosest = () => {
      if (!navigator.geolocation) {
        if (status) status.textContent = 'Choose a region to zoom the map.';
        applyFilter('closest');
        return;
      }
    ...
    explorer.querySelectorAll('[data-location-card]').forEach((button) => {
      button.addEventListener('click', () => {
        const id = parseInt(button.getAttribute('data-location-card'), 10);
        if (!id) return;
        cards.forEach((card) => card.classList.toggle('is-active', parseInt(card.getAttribute('data-location-id'), 10) === id));
        window.chromaLocationExplorerState = window.chromaLocationExplorerState || {};
        window.chromaLocationExplorerState[mapId] = Object.assign(
          {},
          window.chromaLocationExplorerState[mapId] || {},
