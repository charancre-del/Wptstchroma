...
function chroma_contact_corporate_meta_box_render( $post ) {
	...
	$corporate_email   = get_post_meta( $post->ID, 'contact_corporate_email', true );
	?>
	<table class="form-table">
		<tr>
		...
		<tr>
			<th><label for="contact_corporate_email">Email Address</label></th>
			<td>
				<input type="email" id="contact_corporate_email" name="contact_corporate_email"
					   value="<?php echo esc_attr( $corporate_email ); ?>"
					   class="large-text" placeholder="e.g., info@chromaela.com" />
				<p class="description">Falls back to the global company email when left empty.</p>
			</td>
		</tr>
	</table>
	<?php
...
function chroma_save_contact_page_meta( $post_id ) {
	...
	$meta_boxes = array(
		...
		'chroma_contact_corporate_nonce' => array(
			...
			'contact_corporate_email'   => 'sanitize_email',
		),
...
function chroma_seed_contact_page_defaults( $post_id ) {
	...
	$defaults = array(
		...
		'contact_corporate_name'    => get_bloginfo( 'name' ),
		'contact_corporate_address' => function_exists( 'chroma_global_full_address' ) && chroma_global_full_address() ? chroma_global_full_address() : "3554 Old Milton Pkwy\nAlpharetta, GA 30005",
		'contact_corporate_phone'   => function_exists( 'chroma_global_phone' ) && chroma_global_phone() ? chroma_global_phone() : '470-470-6589',
		'contact_corporate_email'   => function_exists( 'chroma_global_email' ) && chroma_global_email() ? chroma_global_email() : 'info@chromaela.com',
