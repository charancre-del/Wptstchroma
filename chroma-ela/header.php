...
<header class="sticky top-0 z-40 bg-white/85 backdrop-blur-xl border-b border-chroma-blue/10">
	<div class="max-w-7xl mx-auto px-4 lg:px-6 h-[82px] flex items-center justify-between">
		...
		<!-- CTA Button -->
		<?php
		$cta_text = get_field( 'header_cta_text', 'option' ) ?: 'Book A Tour';
		$cta_link = get_field( 'header_cta_link', 'option' ) ?: '#tour';
		?>
		<a href="<?php echo esc_url( $cta_link ); ?>" class="hidden sm:inline-flex items-center gap-2 bg-brand-ink text-white text-xs font-semibold tracking-[0.2em] px-5 py-3 rounded-full shadow-soft hover:bg-chroma-blueDark">
			<?php echo esc_html( $cta_text ); ?>
		</a>

		<!-- Mobile Menu Button -->
		<button id="menu-btn" class="md:hidden text-2xl text-brand-ink" aria-label="Open menu">☰</button>
	</div>
	...
	<div id="mobile-menu" class="fixed inset-0 bg-white z-50 translate-x-full transition-transform duration-300 md:hidden flex flex-col">
		<div class="flex items-center justify-between px-5 py-5 border-b border-chroma-blue/10">
			...
			<button id="close-menu" class="text-3xl text-brand-ink" aria-label="Close menu">×</button>
		</div>
		<nav class="flex-1 px-6 py-6 text-lg font-semibold text-brand-ink space-y-6">
			...
			?>
			<a href="<?php echo esc_url( $cta_link ); ?>" class="block bg-brand-ink text-white text-center py-3 rounded-2xl shadow-soft hover:bg-chroma-blueDark transition mt-4">
				<?php echo esc_html( $cta_text ); ?>
			</a>
		</nav>
	</div>
