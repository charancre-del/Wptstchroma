...
while (have_posts()):
	...
	if ($hero_image_override !== '') {
		...
		$hero_image_markup = wp_get_attachment_image(
			...
			array(
				...
				'alt' => esc_attr__('Five-year-old child concentrating while writing letters in a workbook', 'chroma-excellence'),
			)
		);
	}
				...
				<?php else: ?>
					<div class="w-full h-full bg-gradient-to-br from-chroma-blueLight via-white to-brand-cream flex items-center justify-center p-10 text-center">
						<div>
							...
							<p class="font-serif text-3xl text-brand-ink mb-3"><?php esc_html_e('Kindergarten readiness begins in summer.', 'chroma-excellence'); ?></p>
		...
		<div class="max-w-6xl mx-auto px-4 lg:px-6">
			<div class="grid lg:grid-cols-2 gap-16 items-center">
				<div class="bg-brand-cream rounded-[3rem] p-8 shadow-soft border border-brand-ink/5 order-2 lg:order-1 rising-k-chart-shell">
					<canvas id="risingKindergartenChart" role="img" aria-label="<?php esc_attr_e('Radar chart showing the Rising Kindergarten Prismpath focus areas: 95% Academic, 80% Social, 75% Emotional, 60% Creative, and 50% Physical.', 'chroma-excellence'); ?>">
						<?php esc_html_e('A radar chart illustrating our curriculum heavy focus on Academic readiness and Executive Function for Kindergarten.', 'chroma-excellence'); ?>
					</canvas>
				</div>
	...
	<script>
		document.addEventListener('DOMContentLoaded', function () {
			const canvas = document.getElementById('risingKindergartenChart');
