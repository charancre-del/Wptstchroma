 ...
 * 
 * Page template for displaying all cities/communities.
 * Create a page and select "Communities" template to use this.
 *
 ...
 */
function chroma_page_get_city_image($city_post, $fallback_url)
{
 ...
 */
function chroma_page_get_city_description($city_post, $max_length = 60)
{
...
<section class="relative bg-white pt-12 pb-16 overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 lg:px-6 relative z-10 text-center">
        ...
        <h1 class="font-serif text-4xl md:text-6xl font-bold text-brand-ink mb-6"><?php the_title(); ?></h1>
...
<div class="max-w-7xl mx-auto px-4 lg:px-6 py-12 space-y-20">
    ...
    <?php foreach ($cities_by_county as $county_name => $county_cities):
        $county_slug = sanitize_title($county_name);
        ...
        <section id="county-<?php echo esc_attr($county_slug); ?>" class="scroll-mt-32">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php foreach ($county_cities as $city_post):
                    ...
                    // Get city image
                    $city_image = chroma_page_get_city_image($city_post, $local_fallback);

                    // Check status
                    $is_opening_soon = ($campus_count === 0);

                    // Get description
                    $city_description = chroma_page_get_city_description($city_post);
                    ?>
