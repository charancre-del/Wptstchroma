...
<head>
  ...
  <style>
    ...
    .post-content h3 { font-family: 'Playfair Display', serif; font-size: 1.5rem; font-weight: 700; color: #263238; margin-bottom: 1rem; margin-top: 3rem; }
    .post-content p { margin-bottom: 1.5rem; }
    .post-content p:first-of-type::first-letter { font-size: 3rem; font-family: 'Playfair Display', serif; color: #4A6C7C; float: left; margin-right: 0.75rem; margin-top: -0.375rem; line-height: 1; }
    .post-content blockquote { border-left: 4px solid #E6BE75; padding-left: 1.5rem; font-style: italic; font-size: 1.25rem; color: #263238; margin: 2.5rem 0; }
    .post-content ul { list-style: disc; padding-left: 1.5rem; margin-bottom: 1.5rem; }
    .post-content ul li { margin-bottom: 0.5rem; }
    .post-content .callout-box { background: white; padding: 2rem; border-radius: 1.5rem; border: 1px solid rgba(38, 50, 56, 0.1); margin: 3rem 0; }
    .post-content .callout-box h4 { font-weight: 700; font-size: 1.125rem; margin-bottom: 1rem; margin-top: 0; }
    .post-content .callout-box ul { margin-bottom: 0; }
  </style>
  <?php wp_head(); ?>
</head>
  ...
  <header class="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-chroma-blue/10">
    <div class="max-w-7xl mx-auto px-4 lg:px-6 h-[82px] flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-8 text-sm font-semibold text-brand-ink/70">
        ...
        ?>
        <a href="<?php echo esc_url( $stories_url ); ?>" class="hover:text-chroma-blue flex items-center gap-2"><i class="fa-solid fa-arrow-left"></i> Back to Stories</a>
      </nav>
      <?php
      $locations_page = get_page_by_path( 'locations' );
      $locations_url = $locations_page ? get_permalink( $locations_page->ID ) : home_url( '/locations/' );
      ?>
      <a href="<?php echo esc_url( $locations_url ); ?>" class="hidden sm:inline-flex items-center gap-2 bg-brand-ink text-white text-xs font-semibold tracking-[0.2em] px-6 py-3 rounded-full shadow-soft">Book Tour</a>
    </div>
  </header>
    ...
    <article>
      <header class="py-20 text-center max-w-4xl mx-auto px-4">
        ...
        <div class="flex items-center justify-center gap-4">
          <img src="<?php echo esc_url( $author_avatar ); ?>" class="w-12 h-12 rounded-full object-cover border-2 border-white shadow-md" alt="<?php echo esc_attr( $author_name ); ?>" />
          <div class="text-left">
      ...
      <div class="max-w-6xl mx-auto px-4 mb-16">
        <div class="rounded-[3rem] overflow-hidden shadow-xl h-[500px]">
          <img src="<?php echo esc_url( $featured_image ); ?>" class="w-full h-full object-cover" alt="<?php the_title_attribute(); ?>" />
        </div>
      </div>
      <?php endif; ?>

      <div class="max-w-3xl mx-auto px-4 pb-24 leading-relaxed text-lg text-brand-ink/80 post-content">
        <?php the_content(); ?>
      </div>
    </article>
