...
<body class="bg-brand-cream text-brand-ink antialiased flex flex-col min-h-screen">
  ...
  <main class="flex-grow flex flex-col items-center justify-center text-center px-4">
    ...
    <h1 class="text-4xl md:text-5xl font-serif font-bold text-brand-ink mb-6">Ruh-roh! This page is playing hide-and-seek.</h1>
    <p class="text-lg text-brand-ink/60 max-w-md mb-10">We've checked the toy bin, looked under the rugs, and even asked the goldfish, but we can't find this page anywhere. It must be really good at hiding!</p>

    <div class="flex flex-wrap justify-center gap-4">
      <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="px-8 py-3 bg-brand-ink text-white font-bold rounded-full uppercase tracking-widest text-xs hover:bg-chroma-blue transition-colors">Go Home</a>
      <?php
      ...
      ?>
      <a href="<?php echo esc_url( $locations_url ); ?>" class="px-8 py-3 bg-white border border-brand-ink/10 text-brand-ink font-bold rounded-full uppercase tracking-widest text-xs hover:border-chroma-blue hover:text-chroma-blue transition-colors">Find a School</a>
    </div>
  </main>
