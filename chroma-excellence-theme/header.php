...
<body <?php body_class( 'bg-brand-cream text-brand-ink antialiased selection:bg-chroma-teal selection:text-white' ); ?>>
<?php wp_body_open(); ?>

<header class="sticky top-0 z-40 bg-white/85 backdrop-blur-xl border-b border-brand-navy/10">
	<div class="max-w-7xl mx-auto px-4 lg:px-6 h-[82px] flex items-center justify-between">
		...
		<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="flex items-center gap-3 group">
			<div class="flex -space-x-1">
				...
				<span class="w-3 h-3 rounded-full bg-chroma-teal opacity-90"></span>
			</div>
			<div class="leading-tight">
				<p class="font-bold text-lg text-brand-ink">Chroma</p>
				<p class="text-[10px] uppercase tracking-[0.2em] text-brand-navy">Early Learning</p>
			</div>
		</a>
		...
		<!-- CTA Button -->
		<a href="<?php echo esc_url( home_url( '/contact#tour' ) ); ?>" class="hidden sm:inline-flex items-center gap-2 bg-brand-ink text-white text-xs font-semibold tracking-[0.2em] px-5 py-3 rounded-full shadow-soft hover:bg-brand-navy">
			Book A Tour
		</a>
	...
	<div data-mobile-nav class="fixed inset-0 bg-white z-50 translate-x-full transition-transform duration-300 md:hidden flex flex-col">
		<div class="flex items-center justify-between px-5 py-5 border-b border-brand-navy/10">
		...
		<nav class="flex-1 px-6 py-6 text-lg font-semibold text-brand-ink space-y-6">
			<?php chroma_primary_nav(); ?>
			<a href="<?php echo esc_url( home_url( '/contact#tour' ) ); ?>" class="block bg-brand-ink text-white text-center py-3 rounded-2xl shadow-soft hover:bg-brand-navy transition mt-4">
				Book A Tour
			</a>
		</nav>
	</div>
