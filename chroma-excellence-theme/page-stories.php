	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<section class="py-12 px-4 lg:px-6 max-w-7xl mx-auto">
			<a href="<?php echo esc_url( get_permalink( $featured_post_id ) ); ?>"
			   class="block relative rounded-[3rem] overflow-hidden shadow-soft group cursor-pointer h-[500px]">
				...
				<div class="absolute bottom-0 left-0 p-8 md:p-12">
					<span class="bg-chroma-yellow text-brand-ink text-[10px] font-bold uppercase px-3 py-1 rounded-full mb-4 inline-block">
						Featured
					</span>
					...
					<span class="text-white text-xs font-bold uppercase tracking-widest border-b border-white/40 pb-1">
						Read Story
					</span>
				</div>
			</a>
			...
			<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
				...
				<article class="group cursor-pointer">
					<a href="<?php the_permalink(); ?>" class="block">
						...
						<h3 class="font-serif text-xl font-bold text-brand-ink mt-2 mb-2 group-hover:text-<?php echo esc_attr( $category_color ); ?> transition-colors">
							<?php the_title(); ?>
						</h3>
